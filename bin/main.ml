let main =
  ...
  let definition = TryCoq.Utils.File.read_file_from_path definition in
  let axiom = TryCoq.Utils.File.read_file_from_path axiom in
  let program_a = TryCoq.Utils.File.read_file_from_path program_a in
  let program_b = TryCoq.Utils.File.read_file_from_path program_b in
  let _ = print_endline "Choose the proof type :" in
  let _ = print_endline "1) Interactive Mode \t 2) Auto Mode" in
  let s = read_line () in
  if s = "1"
  then (
    let _ = TryCoq.Engine.proof_top definition axiom program_a program_b in
    ())
  else (
    let _ = print_string "Enter the goal : " in
    let goal = read_line () in
    let _ = TryCoq.Engine.proof_auto definition axiom program_a program_b goal in
    ())
;;
